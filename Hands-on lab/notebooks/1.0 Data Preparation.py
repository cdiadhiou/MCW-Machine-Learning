...
# MAGIC In this notebook, you will preprocess a raw dataset and store it in Azure Machine Learning.
...
filename='daily-battery-time-series-v3-raw.csv'

downloadCommand = 'wget -O ''/dbfs{0}/{1}'' ''https://mcwmachinelearningsa.blob.core.windows.net/data/{1}'''.format(tempFolderName, filename)
...
# MAGIC %md A simple check to make sure our data file was correctly downloaded.
...
# df = pd.read_csv('mangled.csv', index_col='Date', parse_dates=['Date'])
...
print(f'Entries starts at {df.index[0]}')
print(f'Entries ends at {df.index[-1]}')
